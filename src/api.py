"""FastAPI application for ai-meeting-copilot."""
...
app = FastAPI(title="ai-meeting-copilot", version="0.1.0", description="ai-meeting-copilot API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "ai-meeting-copilot", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "ai-meeting-copilot", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "ai-meeting-copilot", "version": "0.1.0", "ready": True,
